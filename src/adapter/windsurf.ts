...
const TARGET_FILE = '.windsurfrules';
const BACKUP_FILE = '.windsurfrules.backup';

function isManagedByAiwright(content: string): boolean {
  return content.startsWith('# AI Prompt Configuration (managed by aiwright)');
}

export class WindsurfAdapter implements AdapterContract {
  readonly name = 'windsurf';
  readonly description = 'Windsurf (.windsurfrules — full ownership)';

  async detect(projectDir: string): Promise<DetectResult> {
    const targetPath = path.join(projectDir, TARGET_FILE);

    if (await fileExists(targetPath)) {
      return { detected: true, confidence: 0.9, reason: '.windsurfrules file found' };
    }
    return { detected: false, confidence: 0, reason: '.windsurfrules not found' };
  }
