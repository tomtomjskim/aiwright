...
const BACKUP_FILE = '.cursorrules.backup';

function isManagedByAiwright(content: string): boolean {
  return content.startsWith('# AI Prompt Configuration (managed by aiwright)');
}

export class CursorAdapter implements AdapterContract {
  readonly name = 'cursor';
  readonly description = 'Cursor (.cursorrules — full ownership)';

  async detect(projectDir: string): Promise<DetectResult> {
    const cursorrules = path.join(projectDir, TARGET_FILE);
    const cursorDir = path.join(projectDir, '.cursor');

    if (await fileExists(cursorrules)) {
      return { detected: true, confidence: 0.9, reason: '.cursorrules file found' };
    }
    if (await fileExists(cursorDir)) {
      return { detected: true, confidence: 0.8, reason: '.cursor/ directory found' };
    }
    return { detected: false, confidence: 0, reason: 'Neither .cursorrules nor .cursor/ found' };
  }
