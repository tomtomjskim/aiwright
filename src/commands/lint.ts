  ...
  program
    ...
    .action(async (recipeName: string | undefined, opts: { severity: string }) => {
      ...
      if (!validSeverities.includes(severityFilter)) {
        throw new ValidationError('--severity must be HIGH, WARN, or INFO');
      }
      ...
      if (!targetName) {
        ...
        if (keys.length === 0) {
          throw new CommandError('No recipes defined.');
        }
        targetName = keys[0];
      }
      ...
      if (!recipeEntry) {
        throw new RecipeNotFoundError(targetName);
      }
      ...
      if (high > 0) throw new CommandError('Lint found HIGH severity issues', 2);
    });
}
